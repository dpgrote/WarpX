    ...
    if (!initialized)
    {
        ...
        if (nspecies > 0) {
            ...
            if (!replenish_species.empty()) {
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(((static_cast<int>(replenish_species.size())) % 2) == 0,
                    "The number of replenishment species list must be even");
                for (auto const& name : replenish_species) {
                    auto it = std::find(species_names.begin(), species_names.end(), name);
                    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                        it != species_names.end(),
                        "ERROR: species '" + name
                        + "' in particles.replenish_species must be part of particles.species_names"
                    );
                    int i = std::distance(species_names.begin(), it);
                    m_species_to_replenish.push_back(i);
                }
                ...
                utils::parser::queryArrWithParser(pp_particles, "replenish_range", replenish_range, 0, 2);
                if (replenish_range.size() > 0) {
                    ...
                    replenish_xmin = replenish_range[0];
                    replenish_xmax = replenish_range[1];
                }
            }
...
MultiParticleContainer::InitializeReplenishNumber ()
{
    if (m_species_to_replenish.size() > 0) {
        ...
        m_total_number_of_particles_for_replenishing.resize(n_replenish);
        for (int i=0 ; i < n_replenish ; i++) {
            auto const& pc = allcontainers[m_species_to_replenish[i*2]];
            m_total_number_of_particles_for_replenishing[i] = pc->TotalNumberOfParticles();
        }
    }
}
...
{
    WARPX_PROFILE("MultiParticleContainer::ReplenishPlasma()");
    if (m_species_to_replenish.size() == 0) {
        return;
    }
    ...
    for (int i_replenish=0 ; i_replenish < n_replenish ; i_replenish++) {
        ...
        // Parameters are based off of the even species in the list,
        // which is the ones whose number will be kept fixed.
        const int i_species = m_species_to_replenish[i_replenish*2];
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
        ...
        } // Loop over particle iter

    } // Loop over pairs of species

    /* Redistribute(); // Not needed since will be done afterwards in main loop. */
}
